...
def teamstats_insertion_sort(teamstat_list):
    for sort_range in range(1, len(teamstat_list)):
        for current_index in range(sort_range, 0, -1):
            ...
            if (
                teamstat_list[previous_index].points
                < teamstat_list[current_index].points
            ):
                break
            if (
                teamstat_list[previous_index].points
                == teamstat_list[current_index].points
                and teamstat_list[previous_index].goal_difference
                < teamstat_list[current_index].goal_difference
            ):
                break

            teamstat_list[previous_index], teamstat_list[current_index] = (
                teamstat_list[current_index],
                teamstat_list[previous_index],
            )
...
for teamstat in sorted_teamstats:
    print(
        f"['{teamstat.name}', {{'points': {teamstat.points}}}, {{'gd': {teamstat.goal_difference}}}]"
    )
