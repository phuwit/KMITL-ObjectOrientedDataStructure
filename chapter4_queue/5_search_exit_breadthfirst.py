...
class Queue:
    def __init__(self, items: List[Coord]) -> None:
        self.__queue = items

    def dequeue(self) -> Coord:
        return self.__queue.pop(0)

    def enqueue(self, item: Coord) -> None:
...
def get_nearby_coords(_coord: Coord, room_width: int, room_height: int):
...
def check_coord(_room: List[str], _width: int, _height: int, _coord: Coord):
    # _coord = Coord(_coord.y - 1, _coord.x - 1)
    if _room[_coord.y][_coord.x] == EXIT_SYMBOL:
        return True
